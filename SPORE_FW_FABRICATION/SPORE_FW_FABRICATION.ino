...
void setup() {
  ...
  while(digitalRead(BOOTLOAD_PIN)) {
    for (uint16_t i = 0; i < pixels.PixelCount(); i++) {
      RgbColor color = HsbColor(testHue/1000.0f, 1.0f, 1.0f);     // h, s, b
      //strip.SetPixelColor(i, color);
      pixels.SetPixelColor(i, colorGamma.Correct(color));
    }
    pixels.Show();
    
    static unsigned long lastUpdate;
    if (millis() - lastUpdate > 17) {         // 17 = ~60fps
      lastUpdate = millis();
      testHue = (testHue==1000.0f) ? 0.0f : (testHue+=1.0f);
      Serial.printf("\n");
    }
    Serial.printf(".");
  }

...
Preferences preferences;                //Preferences replaces EEPROM for ESP32
...
#define LED_BUILTIN   16                 // share RX on ESP32
...
void setup() {
  ...
  preferences.begin("spore-config", false);
  //EEPROM.get(0, address);
  //address = EEPROM.read(0);
  address =  preferences.getUInt("address", 0); //get address value, default 0
  deviceName.concat(address);
//  for (int i = 0; i < 4; i++) {
//    serverIP[i] = EEPROM[i + 1];  // + 1 because "address" is stored at 0
//  }
  serverIP=preferences.getString("serverIP","10.10.10.100");
  ...
  //currentMode = TEST;
  //currentMode = (Mode)EEPROM[5]; //EEPROM
  currentMode = preferences.getString("mode") //get mode (TEST, SLEEP, NORMAL), default NORMAL
